...
export const determineAIAction = (
  ...
  switch (difficulty) {
    case 'easy':
      return determineEasyAIAction(enemyHand, enemyField, playerField, enemyEnergy);
    case 'medium':
      return determineMediumAIAction(enemyHand, enemyField, playerField, enemyEnergy);
    case 'hard':
      return determineHardAIAction(enemyHand, enemyField, playerField, enemyTools, enemySpells, enemyEnergy);
    case 'expert':
      return determineExpertAIAction(enemyHand, enemyField, playerField, enemyTools, enemySpells, enemyEnergy);
    default:
      return determineEasyAIAction(enemyHand, enemyField, playerField, enemyEnergy);
  }
};
...
const determineEasyAIAction = (enemyHand, enemyField, playerField, enemyEnergy) => {
  // If no creatures on field and have cards in hand, deploy one
  if (enemyField.length === 0 && enemyHand.length > 0) {
  ...
  // If have creatures in hand and energy and field isn't full, maybe deploy a creature
  if (enemyHand.length > 0 && enemyField.length < 3) {
    // 60% chance to deploy
    if (Math.random() < 0.6) {
      const randomCreature = enemyHand[Math.floor(Math.random() * enemyHand.length)];
...
const determineMediumAIAction = (enemyHand, enemyField, playerField, enemyEnergy) => {
  // Deploy strongest creature from hand if field isn't full
  if (enemyField.length < 3 && enemyHand.length > 0) {
...
const determineHardAIAction = (enemyHand, enemyField, playerField, enemyTools, enemySpells, enemyEnergy) => {
...
const determineExpertAIAction = (enemyHand, enemyField, playerField, enemyTools, enemySpells, enemyEnergy) => {
